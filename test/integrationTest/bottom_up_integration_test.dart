...
void main() {
  ...
  group('Level 1 — DAO + SQLite', () {
    test(
      'ItemDao persists and retrieves an item with its transaction',
      () async {
        await _seedFkDeps();
  ...
  group('Level 2 — Repository + DAO', () {
    ...
    test(
      'fetchUserItems fetches server data and persists it to SQLite',
      () async {
        await _seedFkDeps();
  ...
  group(
    'Level 3 — ViewModel + Repository + DAO (no network, SQLite fallback)',
    () {
      setUp(() {
      ...
      test(
        'fetchItems serves data from SQLite when server is unreachable',
        () async {
          await _seedFkDeps();
